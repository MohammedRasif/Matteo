	...
	const handleSubmit = async (e) => {
		...
		try {
			const response = await fetch(
				`${BaseUrl}/api/v1/order-post/create/`,
				{
					method: "POST",
					headers: {
				...
				<form onSubmit={handleSubmit} className="space-y-4">
					<div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-[#154153]">
						<div className="md:col-span-1">
							...
							<input
								...
								required
								className="w-full px-3 py-2 border border-[#848239] bg-[#F8FCFF] rounded"
							/>
						</div>
						<div className="md:col-span-1">
							<label className="block text-[16px] mb-2 text-[#154153]">
					...
					<div>
						...
						<textarea
							...
							className="w-full h-[96px] px-3 py-2 border border-[#848239] bg-[#F8FCFF] rounded"
						/>
					</div>
					...
					<div className="rounded-[8px] mx-10 border my-8 border-[#5285A7] relative">
						<div className="absolute top-4 left-4 right-4 z-10 flex items-center justify-between p-4">
							<div className="text-xs text-[#848239] py-5 bg-[#EEF2F5] px-10 rounded-[8px]">
								(Task location)
							</div>
							<div className="flex items-center rounded overflow-hidden flex-1 mx-4 relative">
								...
								<input
									...
									className="text-sm pl-8 h-[55px] w-full mx-auto bg-[#F8FCFF] border border-[#848239] rounded-[8px] focus:outline-none"
								/>
							</div>
							<button className="cursor-pointer text-xs text-white py-5 bg-[#848239] border border-[#848239] px-10 rounded-[8px]">
								Auto locate
							</button>
						</div>
					...
					<div className="flex justify-between gap-10 items-center">
							...
							<div className="flex items-center space-x-4 mt-2">
								<div className="flex items-center space-x-2">
									<input
										...
										checked={hourlyRate}
										onChange={(e) =>
											setHourlyRate(e.target.checked)
										}
										className="rounded border-[#848239] bg-[#F8FCFF]"
									/>
						...
						<div className="flex w-full">
							<input
								...
								className="rounded-l border-l border-y border-[#848239] bg-[#F8FCFF] p-2 placeholder-[#154153] w-full"
							/>
							<div className="flex items-center rounded border border-[#848239] bg-[#F8FCFF] text-[#154153] p-2 w-1/2">
								Currency ({currency})
							</div>
						</div>
					</div>
					...
					<div className="pt-10 text-center">
						<button
							type="submit"
							className="w-[231px] hover:cursor-pointer bg-[#848239] text-white text-[20px] text-center py-2 rounded transition-colors"
						>
							Submit
						</button>
					</div>
