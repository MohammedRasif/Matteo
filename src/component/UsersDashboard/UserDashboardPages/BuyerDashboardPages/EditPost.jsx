...
function EditPost() {
		...
		const fetchPost = async () => {
			try {
				const res = await fetch(
					`${BaseUrl}/api/v1/order-post/details/${id}/`,
					{
	...
	const getChangedFields = () => {
		...
		if (
			formData.duration &&
			Number(formData.duration) !== initialData.duration
		)
	...
	const handleSubmit = async (e) => {
		...
		try {
			const res = await fetch(
				`${BaseUrl}/api/v1/order-post/update/${id}/`,
				{
					method: "PATCH",
					headers: {
			...
			toast.success("Updated successfully!", {
				duration: 1000, // Toast displays for 2 seconds
			});
