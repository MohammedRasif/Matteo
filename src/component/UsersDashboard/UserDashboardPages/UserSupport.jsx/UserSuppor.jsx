...
import { useSupportTicketQuery, useSupportTicketDeleteMutation, useSupporTicketPostMutation, useSupportTikectEditMutation } from "../../../../Redux/feature/ApiSlice";
...
function UserSupport() {
  ...
  const { data: supportTicket, isLoading, isError } = useSupportTicketQuery();
  ...
  const toggleActionDropdown = (ticketId) => {
    setOpenActionDropdownId(openActionDropdownId === ticketId ? null : ticketId);
  };

  const togglePurposeDropdown = () => setIsPurposeDropdownOpen(!isPurposeDropdownOpen);
  ...
  const filteredTickets = supportTicket
    ? selectedFilter === "All"
      ...
      : supportTicket.filter(
        (ticket) => ticket.purpose.toLowerCase() === selectedFilter.toLowerCase()
      )
    : [];
  ...
  const handleCreateOrUpdateTicket = async () => {
    ...
    try {
      if (isEditMode) {
        ...
        await supportTicketEdit({ id: editTicketId, question: ticketData }).unwrap();
      } else {
      ...
      console.error(isEditMode ? "Failed to update ticket:" : "Failed to create ticket:", error);
      alert(isEditMode ? "Failed to update ticket. Please try again." : "Failed to create ticket. Please try again.");
    }
  };
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (
        ...
        !actionDropdownRefs.current[openActionDropdownId].contains(event.target)
      ) {
      ...
      if (
        ...
        !modalRefs.current[`delete_modal_${deleteTicketId}`].contains(event.target) &&
        document.getElementById(`delete_modal_${deleteTicketId}`)?.open
      ) {
        document.getElementById(`delete_modal_${deleteTicketId}`).close();
          ...
          {isOpenDropdown && (
            <div className="absolute z-10 bg-[#F6F8FA] rounded mt-2 p-2 shadow-2xl w-full">
              <ul className="space-y-1 text-[16px] text-[#012939]">
                ...
                <li
                  className="hover:bg-gray-100 p-1 rounded cursor-pointer flex items-center"
                  onClick={() => handleFilterSelect("Withdrawal")}
                >
                  Withdrawal <MdOutlineKeyboardArrowRight />
                </li>
                <li
                  className="hover:bg-gray-100 p-1 rounded cursor-pointer flex items-center"
                  onClick={() => handleFilterSelect("Blog related")}
                >
                  Blog related <MdOutlineKeyboardArrowRight />
                </li>
                <li
                  className="hover:bg-gray-100 p-1 rounded cursor-pointer flex items-center"
                  onClick={() => handleFilterSelect("Order related")}
                >
            ...
            <div className="modal-box">
              <div className="modal-box relative bg-[#EFF2F6] w-[470px] p-6 text-[#154153]">
                <div className="modal-action">
                  <form method="dialog">
                    <button className="btn btn-sm btn-circle hover:cursor-pointer" onClick={handleCloseModal}>
                ...
                <h1 className="text-center text-[20px] font-medium">{isEditMode ? "Edit Ticket" : "Support Ticket"}</h1>
                      ...
                      <div className="absolute z-10 bg-[#F6F8FA] rounded mt-1 p-2 shadow-2xl w-full">
                        <ul className="space-y-1 text-[16px] text-[#012939]">
                          <li
                            ...
                            onClick={() => handlePurposeSelect("withdrawal")}
                          >
                            Withdrawal <MdOutlineKeyboardArrowRight />
                          </li>
                          <li
                            className="hover:bg-gray-100 p-1 rounded cursor-pointer flex items-center justify-between"
                            onClick={() => handlePurposeSelect("blog")}
                          >
                            Blog related <MdOutlineKeyboardArrowRight />
                          </li>
                          <li
                            className="hover:bg-gray-100 p-1 rounded cursor-pointer flex items-center justify-between"
                            onClick={() => handlePurposeSelect("order")}
                          >
                            Order related <MdOutlineKeyboardArrowRight />
                          </li>
                        </ul>
                ...
                <div className="text-center">
                  <button
                    className="btn btn-active bg-[#0D95DD] p-4 rounded-2xl w-[120px] font-bold text-[17px] text-[#FFFFFF] cursor-pointer"
        ...
        <div className="my-4">
          <table className="w-full border-collapse">
            ...
            <tbody>
              {isLoading ? (
                <tr>
                  <td colSpan="6" className="py-3 px-4 text-center text-[#012939]">
                    Loading...
                  </td>
                </tr>
              ) : isError ? (
                ...
                filteredTickets.map((ticket) => (
                  <tr key={ticket.id} className="text-[#012939]">
                    <td className="py-3 px-4 capitalize">{ticket.purpose}</td>
                    <td className="py-3 px-4 text-[#0D95DD] underline cursor-pointer">
                      <button
                        className="btn"
                        onClick={() => document.getElementById(`my_modal_${ticket.id}`).showModal()}
                      >
                        View
                      </button>
                      <dialog
                        ...
                        ref={(el) => (modalRefs.current[`my_modal_${ticket.id}`] = el)}
                      >
                        <div className="modal-box">
                          <div className="modal-action">
                            <form method="dialog">
                          ...
                          <div className="space-y-3">
                            <h3 className="text-lg text-[#0D95DD] text-[20px] text-center font-medium">
                              <span>Ticket</span> ({ticket.id})
                            </h3>
                            <h4 className="font-medium text-[#154153]">Details</h4>
                            <p className="py-4 text-[#154153]">{ticket.details}</p>
                          </div>
                        </div>
                      </dialog>
                    </td>
                    ...
                    <td className="py-3 px-4 relative cursor-pointer">
                      <div
                        ...
                        onClick={() => toggleActionDropdown(ticket.id)}
                   
                      >
                        <span className="text-[#012939] font-bold text-[18px]">...</span>
                      </div>
                      {openActionDropdownId === ticket.id && (
                        <ul
                      ...
                      <dialog
                        ...
                        ref={(el) => (modalRefs.current[`delete_modal_${ticket.id}`] = el)}
                      >
                        <div className="fixed inset-0 flex items-center justify-center z-50 backdrop-blur-[3px]">
                          <div className="bg-white rounded-lg p-6 max-w-md w-full">
                            <div className="flex items-center justify-between mb-4">
                              ...
                              <button
                                onClick={() => {
                                  document.getElementById(`delete_modal_${ticket.id}`).close();
                                  setDeleteTicketId(null);
                                }}
                            ...
                            <div className="flex justify-end gap-2">
                              <button
                                onClick={() => handleCancelDelete(ticket.id)}
