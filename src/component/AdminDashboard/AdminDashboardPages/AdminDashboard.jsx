...
const AdminDashboard = () => {
    ...
    const chartData = SelasOverview?.map((item) => ({
        name: item.month,
    ...
    const barChartData = subscription?.map((item) => ({
        name: item.date,
        ...
        <div className="p-5 mt-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div className="bg-white rounded-lg p-4 flex justify-between items-center">
                    <div>
                        ...
                        <h3 className="text-2xl font-semibold mt-1">${todayData?.today_sales}</h3>
                    </div>
                ...
                <div className="bg-white rounded-lg p-4 flex justify-between items-start">
                    <div>
                        ...
                        <h3 className="text-2xl font-semibold mt-1">{totalUser?.total_user}</h3>
                    </div>
            ...
            <div className="flex gap-4">
                    ...
                    <div style={{ height: "330px" }}>
                        <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={chartData}>
                                ...
                                <YAxis
                                    domain={[0, Math.max(...chartData.map((d) => Math.max(d.sells2024, d.sells2025)))]}
                ...
                <div className="bg-white rounded-lg p-4 h-[405px] w-2/5">
                    <h3 className="text-xl font-semibold mb-4">Subscription Statistics (Bar Chart)</h3>
                                    ...
                                    <tr key={index} className="border-b border-gray-100 hover:bg-gray-50">
                                        <td className="py-3 px-4">
                                            <div className="flex items-center">
                                                <span className="text-gray-700">{earner.role}</span>
                                            </div>
                                        </td>
                                        <td className="py-3 px-4 text-gray-700 pl-20">{earner.count}</td>
                                    ...
                                    >
                                        <td className="py-3 px-4">
                                            <div className="flex items-center">
                                                ...
                                                <span className="text-gray-700">{earner.user_name}</span>
                                            </div>
                                        </td>
                                        <td className="py-3 px-4 text-gray-700">{earner.user_id}</td>
                                        <td className="py-3 px-4 text-blue-500 font-medium">
                                            ${earner.total_earnings.toLocaleString("en-US")}
                                        </td>
                                        <td className="py-3 px-4 text-blue-500 font-medium">
                                            ${earner.current_order.toLocaleString("en-US")}
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
