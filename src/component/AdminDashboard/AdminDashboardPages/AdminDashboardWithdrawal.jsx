...
import { useWithdrawalQuery, useWithdrawalWarningMutation } from '../../../Redux/feature/ApiSlice';
...
const AdminDashboardWithdrawal = () => {
    ...
    const { data: withdrawal, isLoading, error } = useWithdrawalQuery();
    ...
    const filteredRequests = (withdrawal || []).filter((request) => {
        ...
        const requestStatus = request.status === 'pending' ? 'In-progress' : request.status;
    ...
    const handleGiveWarning = (request) => {
        ...
        const matchedRequest = (withdrawal || []).find((item) => item.id === request.id);
        if (matchedRequest) {
            ...
            console.error("Request ID not found in withdrawal data:", request.id);
        }
    };
    ...
    const handleWarningSubmit = async () => {
        ...
        if (!warningMessage || warningMessage.trim().length === 0) {
            ...
            console.log("Validation failed: warningMessage is empty or whitespace");
            return;
        }
        ...
        const payload = { warning: warningMessage.trim(), id: selectedRequestId };
    ...
    useEffect(() => {
        const handleClickOutside = (event) => {
            ...
            // Close action menu
            const isOutsideActionMenu = Object.values(actionMenuRefs.current).every(
                                ...
                                <tr key={request.id} className="border-b border-gray-300">
                                    <td className="px-6 py-3 whitespace-nowrap">
                                        <div className="flex items-center">
                                            <img
                                                src={request.view_data.user.image || '/placeholder.svg'}
                                                alt=""
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 relative">
                                        ...
                                        {showActionMenu === request.id && (
                                            <div
                                                ref={(el) => (actionMenuRefs.current[request.id] = el)}
                    ...
                    <div className="bg-gray-100 rounded-lg shadow-lg max-w-xl w-full">
                        ...
                        <div className="px-6 pb-4">
                            <div className="flex items-start">
                                <img
                                    src={selectedRequest.view_data.user.image || '/placeholder.svg'}
                                    alt=""
                                    className="w-24 h-24 rounded-full mr-6"
                                />
                                <div>
                                    <h2 className="text-xl font-medium text-gray-800">
                                        {selectedRequest.view_data.user.username}
                                    </h2>
                                    <p className="text-gray-600">
                                        {selectedRequest.view_data.user.role || 'Hostler'}
                                    </p>
                                    <div className="mt-2">
                                        <p className="text-gray-700">
                                            Rating:{' '}
                                            <span className="font-medium">
                                                {selectedRequest.view_data.rating_count > 0
                                                    ? (
                                                        selectedRequest.view_data.rating_sum /
                                                        selectedRequest.view_data.rating_count
                                                    ).toFixed(1)
                                                    : '0'}
                                            </span>
                                            <span className="text-gray-500">
                                                {' '}
                                                ({selectedRequest.view_data.rating_count} Reviews)
                                            </span>
                                        </p>
                                    </div>
                                </div>
                            ...
                            <div className="mt-6">
                                ...
                                <p className="text-gray-600 text-sm">
                                    {selectedRequest.view_data.user.about || 'No description available'}
                                </p>
                            </div>
                            ...
                            <div className="mt-6 grid grid-cols-2 gap-y-4">
                                <div className="flex items-center space-x-8">
                                    ...
                                    <p className="font-medium">
                                        {selectedRequest.view_data.task_completed_count}
                                    </p>
                                </div>
                                ...
                                <div className="flex items-center space-x-[50px]">
                                    ...
                                    <p className="font-medium">
                                        ${selectedRequest.view_data.task_completed_amount.toFixed(2)}
                                    </p>
                                </div>
                                <div className="flex items-center space-x-8">
                                ...
                                <div className="flex items-center space-x-[90px]">
                                    ...
                                    <p className="font-medium">
                                        {selectedRequest.view_data.user.gender || 'N/A'}
                                    </p>
                                </div>
                                <div className="flex items-center space-x-9">
                                    <p className="text-gray-600">Experience:</p>
                                    <p className="font-medium">
                                        {selectedRequest.view_data.user.years_of_experience
                            ...
                            <div className="mt-6">
                                ...
                                <p className="text-gray-700">
                                    {selectedRequest.view_data.user.skills.length > 0
                            ...
                            <div className="mt-6 flex">
                                ...
                                <p className="text-gray-700 font-medium">
                                    {selectedRequest.view_data.user.languages.length > 0
                    ...
                    >
                        ...
                        <div className="mb-6">
                            {warningStatus === 'success' ? (
                                <p className="text-green-600 text-sm">Warning sent successfully!</p>
                            ) : warningStatus === 'error' ? (
                                <p className="text-red-600 text-sm">Error: {warningError}</p>
                            ) : (
                                <>
                                    <textarea
                                    ...
                                    />
                                    {warningError && (
                                        <p className="text-red-600 text-sm mt-2">{warningError}</p>
                                    )}
                                    {warningStatus === 'loading' && (
                                        <p className="text-gray-600 text-sm mt-2">Sending warning...</p>
                                    )}
                                </>
                            )}
                        </div>
                        ...
                        <div className="flex justify-center space-x-4">
                            ...
                            >
                                {warningStatus === 'success' ? 'Close' : 'Cancel'}
                            </button>
                        </div>
                    </div>
                </div>
