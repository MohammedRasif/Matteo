      ...
      <div className="flex flex-col lg:flex-row gap-3 h-screen" style={{ height: "78vh" }}>
        ...
        <div className="lg:w-1/2 bg-white rounded-lg p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-blue-500 underline text-sm cursor-pointer" onClick={openPopup}>
              View
            </span>
          </div>
          ...
          <p className="text-sm text-gray-600 mb-4">
            Budget: ${managementAssessOrderDetails.service_details.price}
          </p>
          ...
          <p className="text-sm text-blue-600">
            {managementAssessOrderDetails.service_details.user.skills.length > 0
              ? managementAssessOrderDetails.service_details.user.skills.map((skill, index) => (
                  <span key={index}>
                    {skill}
                    {index < managementAssessOrderDetails.service_details.user.skills.length - 1
                      ? ", "
                      : ""}
                  </span>
                ))
        ...
        <div className="lg:w-1/2 bg-white rounded-lg shadow-md">
          ...
          <div className="flex justify-between items-center mb-4 p-3 rounded-md bg-[#E2F2FC]">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-gray-800 pl-1">{buyer.username}</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <img
              ...
              <div>
                ...
                <p className="text-sm font-medium text-gray-800 pl-[1px]">{seller.username}</p>
              </div>
            </div>
          </div>
          ...
          <div className="p-6 flex flex-col-reverse space-y-4 space-y-reverse overflow-y-auto h-[calc(78vh-120px)]">
            {managementAssessOrderDetails.messages.map((message, index) => (
              <div
                key={index}
                className={`flex ${
                  message.sender_username === buyer.username ? "justify-end" : "justify-start"
                }`}
              >
                <div
                  className={`p-3 max-w-xs text-sm ${
                    message.sender_username === buyer.username
