...
function ConfirmPasswordVerification() {
	...
	const [registerVerification, { isLoading }] =
		useRegisterVerificationMutation();
	...
	const handleSubmit = async () => {
		...
		try {
			const res = await registerVerification({
				email,
				otp: otpString,
			}).unwrap();
				...
				<div className="justify-center flex mx-auto rounded md:px-10 px-4 lg:py-20 py-4">
					<div className="text-center lx:space-y-8 space-y-4">
						...
						<button
							...
							disabled={isLoading}
							className={`bg-[#848239] text-[#F6F8FA] px-6 py-2 rounded-[8px] text-[16px] font-bold w-[123px] cursor-pointer ${
			...
			{isModalOpen && (
				...
				>
					<div className="bg-white rounded-lg p-6 w-96 shadow-lg space-y-8 py-10">
						<div className="flex h-16 w-16 items-center justify-center rounded-full mx-auto bg-[#848239] text-white">
							<svg
